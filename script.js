        class BookReader {
            ...
            showAdminLogin() {
                ...
                if (username === 'admin' && password === '123456') {
                    ...
                    this.showAdminPanel();
                    this.showMessage('✅ Đăng nhập Admin thành công!');
                } else {
                    this.showMessage('❌ Sai tên đăng nhập hoặc mật khẩu!');
                }
            }
            ...
            async loadFromGitHub() {
                ...
                try {
                    ...
                    // Load documents.json from GitHub
                    ...
                    if (documents && Array.isArray(documents)) {
                        ...
                        this.hideLoading();
                        this.showMessage(`✅ Đã tải ${documents.length} tài liệu từ GitHub!`);
                        this.closeSettings();
                        
                        // Auto open first document
                        if (documents.length > 0) {
                            setTimeout(() => this.openDocument(0), 500);
                        }
                    } else {
                        throw new Error('Invalid documents.json format');
                    }
            ...
            async uploadFileToGitHub() {
                ...
                try {
                    ...
                    this.showMessage('❌ Lỗi upload: ' + error.message);
                }
            }
 ...
 fixContentEncoding(content) {
    ...
    displayContentDirectly(content) {
        ...
        console.log('📄 Content displayed with proper encoding');
    }
    ...
    async openDocument(index) {
        ...
        try {
            ...
            this.hideLoading();
            this.showError('Lỗi khi tải tài liệu: ' + error.message);
        }
    }
async openDocument(index) {
        console.log('🎯 Opening document:', index);
        ...
        try {
            ...
            if (doc.file) {
                // Build file path
                const filePath = `${this.ghConfig.basePath}/${doc.file}`;
                fileUrl = `https://raw.githubusercontent.com/${this.ghConfig.owner}/${this.ghConfig.repo}/main/${filePath}`;
                ...
                // FIX ENCODING: Thử các encoding khác nhau
                const buffer = await response.arrayBuffer();
                ...
                try {
                    ...
                    // Thử Windows-1252 (cho tiếng Việt cũ)
                    try {
                        const decoder = new TextDecoder('windows-1252');
                        content = decoder.decode(buffer);
                        console.log('✅ Used Windows-1252 encoding');
                    } catch (e2) {
                        // Thử ISO-8859-1
                        const decoder = new TextDecoder('iso-8859-1');
                        content = decoder.decode(buffer);
                        console.log('✅ Used ISO-8859-1 encoding');
                    }
                }
            ...
            // FIX: Đảm bảo encoding UTF-8 trong content
            content = this.fixContentEncoding(content);
    ...
    generateTOC() {
        ...
        const contentDiv = this.fileContentDiv.querySelector('.file-content-display');
        if (!contentDiv) {
            console.log('❌ No content div found for TOC');
            return;
        }
